...
def read_pcap(filename):
    ...
    base_time=0
    #base_time=float(pks[0].sniff_timestamp)
    for pkg in pks:
        if pkg.number=='1':
        ...
        if pkg.highest_layer=='ETH':
            ...
            #                     0            1          2               3            4           5           6_protocol    7     8_host   9_http_info
            ...
            if len(pkg.get_multiple_layers('http')) != 0:
                ...
                if pkg_http.has_field('host'):
                    information[9]['host']=pkg_http.host
                if pkg_http.has_field('request_method'):
                    information[9]['request_method']=pkg_http.request_method
                if pkg_http.has_field('request_version'):
                    information[9]['request_version']=pkg_http.request_version
                if pkg_http.has_field('request_full_uri'):
                    information[9]['request_full_uri']=pkg_http.request_full_uri
                if pkg_http.has_field('user_agent'):
                    information[9]['user_agent']=pkg_http.user_agent
                if pkg_http.has_field('referer'):
                    information[9]['referer']=pkg_http.referer
            ...
            elif pkg.highest_layer=='SMTP':
                information[6]='smtp'
                pkg_smtp=pkg.smtp
                information[9]['content']=[]
                for line in pkg_smtp._get_all_field_lines():
                    information[9]['content'].append(line)
    ...
    information_file = open(os.path.basename(filename)+'_v3.json','w')
