...
def get_other_port(file_name,argv):
    ...
    try:
        ...
        port_hosts=input_stat['port_hosts']
    ...
    for number,information in number_informations.items():
        if information[0] not in src_ips:
            if information[2] not in require_port:
                continue
            ...
            if information[3] not in require_port:
                continue
    ...
    for stream,information in stream__information.items():
        if information[1] =='0':
            print(filename,"find 0 port,stream is ",stream)
        if len(information[4])==0:
            information[4].append(information[0])
        if information[1] not in port_hosts:
            port_hosts[information[1]]=[]
            for host in information[4]:
                if host not in port_hosts[information[1]]:
                    port_hosts[information[1]].append(host)
        else:
            for host in information[4]:
                if host not in port_hosts[information[1]]:
                    port_hosts[information[1]].append(host)
    ...
    port2_txt=open('port2.txt','w')    
    port2_txt.write("所有出现过的端口:\n")
    list_1 = list(port_hosts.items())
    port_hosts_sort= dict(sorted(list_1,key = lambda x:int(x[0])))
    # port2_txt.write(port_hosts_sort.keys())
    for port,hosts in port_hosts_sort.items():
        port2_txt.write(str(port)+",")
    port2_txt.write("\n\n")
    for port,hosts in port_hosts_sort.items():
        port2_txt.write(str(port)+"\n")
        for host in hosts:
            port2_txt.write("\t"+host+"\n")
        port2_txt.write("\n")
    port2_txt.close()
    ...
    output_stat['port_hosts']=port_hosts
