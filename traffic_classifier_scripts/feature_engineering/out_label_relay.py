...
def out_label_packetstream_relay(filename):
    # filename=sys.argv[1]
...
##还有剩下的一些缓存，尝试能不能转成host(即request后都是空tcp，就没办法使用缓存了)
    ...
    for stream,domain in stream_domain.items():
        if domain  not in src_domains:
            ...
            relay=0   ## proxy_packetstream_io --> relay --> 1
        information=number_information[stream_nums[stream][0]]
        out_json.write(json.dumps((relay,information[0],information[1],information[2],information[3],information[4],information[5],information[6],os.path.basename(filename)))+"\n")  
    out_json.close()
    # relay session/connection session, src_ip, src_port, dst_ip, dst_port, start_time, protocol, pcapfilename
...
def out_label_honeygain_relay(filename):
    # filename=sys.argv[1]
...
##还有剩下的一些缓存，尝试能不能转成host(即request后都是空tcp，就没办法使用缓存了)
    ...
    out_json=open(out_flow_dir+'/'+os.path.basename(filename)+'.json','w')
    for stream,domain in stream_domain.items():
        if domain  not in src_domains and domain not in non_need_domains:
            relay=1
        else:
            relay=0
        information=number_information[stream_nums[stream][0]]
...
def out_label_iproyal_relay(filename):
    # filename=sys.argv[1]
...
##还有剩下的一些缓存，尝试能不能转成host(即request后都是空tcp，就没办法使用缓存了)
    ...
    for stream,domain in stream_domain.items():
        ...
        out_json.write(json.dumps((relay,information[0],information[1],information[2],information[3],information[4],information[5],information[6],os.path.basename(filename)))+"\n")
        # else:
            # out_json.write(json.dumps((relay,information[2],information[3],information[0],information[1],information[4],information[5],information[6],os.path.basename(filename)))+"\n")
...
def main():
    if len(sys.argv)==2:
        ...
        out_label_honeygain_relay(sys.argv[1])
